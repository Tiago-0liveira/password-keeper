...
const SteamManagerComponent: React.FC = () => {
	...
	const requestSteamActiveUser = () => { ipcRenderer.send("requestSteamActiveUser") }
	...
	const onStartorStop = (start: boolean, user:string) => () => {
		if (!start) ipcRenderer.send("requestSteamChangeUser", {username: user})
		else ipcRenderer.send("requestSteamShutdown")
