...
const electronAPI = window.electronAPI.apps.passwords
...
const PasswordsComponent: React.FC<PasswordsComponentProps> = (props) => {
	...
	const fetchRows = async () => {
		try {
			let t = false
			while (!t) {
				let rows = await electronAPI?.getRows("", true)
				...
				if (rows === undefined) {
					...
					t = true
				dispatch({ type: PasswordsAppActionType.INITIAL_LOAD, data: rows })
				break
			}
		} catch (err) {
			console.error(err);
	...
	const buttonSaveClick = () => {
		...
		switch (state.bottomBar.state) {
			case EBottomBarState.New:
				electronAPI.validateAndNewRow(state.bottomBar.data.new)
					.then((opt) => {
						if (opt.success) {
							dispatch({type: PasswordsAppActionType.NEW, data: opt.value})
						} else {
							dispatch({type: PasswordsAppActionType.SET_ERROR, data: opt.value})
						}
					}).catch(err => {
						console.error(err)
					})
				break;
			case EBottomBarState.Edit:
				electronAPI.validateAndUpdateRow(data as Row)
					.then((opt) => {
						console.log(opt)
						if (opt.success) {
							state.bottomBar.data.edit?.cancelCallback()
							dispatch({type: PasswordsAppActionType.EDIT, data: opt.value})
						} else {
							dispatch({type: PasswordsAppActionType.SET_ERROR, data: opt.value})
						}
					}).catch(err => {
	...
	const onFilterCheckBoxChange = (propName: string) => (event: React.MouseEvent<HTMLDivElement>) => {
