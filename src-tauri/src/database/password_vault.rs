...
use crate::{dm_error, make_uuid_struct};
...
impl DatabaseModel<Error> for PasswordVaultUser {
    ...
    fn __get_save_params(&self) -> Pair<String, Box<[&dyn rusqlite::ToSql]>> {
        let sql_query = format!(
            "INSERT INTO {} (site, email, password, username) VALUES (?, ?, ?, ?)",
            Self::TABLE_NAME
        );
        let params: Box<[&dyn rusqlite::ToSql]> =
            Box::new([&self.site, &self.email, &self.password, &self.username]);
        Pair(sql_query, params)
    }
    fn __get_update_params(&self) -> Pair<String, Box<[&dyn rusqlite::ToSql]>> {
        let sql_query = format!(
            "UPDATE {} SET site = ?, email = ?, password = ?, username = ? WHERE uuid = ?",
            Self::TABLE_NAME
        );
        let params: Box<[&dyn rusqlite::ToSql]> = Box::new([
            &self.site,
            &self.email,
            &self.password,
            &self.username,
