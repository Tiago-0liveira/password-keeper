...
fn db_get_all<T: DatabaseModel<U>, U: DMValidationError>(
    table_name: &str,
    row_mapper: fn(&rusqlite::Row) -> T,
) -> Result<Vec<T>, Error> {
    ...
    let rows = match stmt.query_map(params!(), |row| Ok(row_mapper(&row))) {
        Ok(rows) => rows,
        Err(err) => return Err(err),
    };
...
fn db_get_one<T: DatabaseModel<U>, U: DMValidationError>(
    table_name: &str,
    row_mapper: fn(&rusqlite::Row) -> T,
    uuid: i32,
) -> Result<T, Error> {
...
pub trait DatabaseModel<T: DMValidationError>: UniquelyIdentified + Serialize {
    ...
    fn get_all() -> Result<Vec<Self>, Error>
    where
        Self: Sized,
    {
        db_get_all(Self::TABLE_NAME, Self::from_row)
    }
    fn get_one(uuid: i32) -> Result<Self, Error>
    where
        Self: Sized,
    {
    ...
    fn save(&mut self) -> Result<(), Either<T, Error>> {
        match self.validate() {
            /* Better validation errors in the future */
            Ok(_) => (),
