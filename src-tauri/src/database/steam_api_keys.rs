...
impl SteamApiKey {
    ...
    pub async fn test_key(&self, steam_id: &str) -> Result<(), Error> {
        if self.api_key.len() != Self::KEY_LEN {
            return Err(Error::InvalidKeyLen);
        } else if !self.api_key.is_valid_steam_api_key() {
        ...
        let client = Client::builder()
            ...
            .unwrap();
        match client
            .get(SteamUser::make_fetch_details_url(
                &self.api_key,
                &vec![&the_chosen_one],
            ))
            .send()
        {
            ...
            Err(err) => Err(Error::ErrorMessage(String::from(err.to_string()))),
        }
    }
}
